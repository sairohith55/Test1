	...
	<c:forEach var="restList" items="${restaurantsList}">
		<tr>
			...
			<c:forEach var="branchList" items="${restList.branchesList}">
				...
				<c:forEach var="cuisineList" items="${branchList.cuisinesList}">
				<td><c:out value="${cuisineList.id}" /></td>
				<td><c:out value="${cuisineList.name}" /></td>
				<td><c:out value="${cuisineList.country}" /></td>
				
				<c:forEach var="recipeList" items="${cuisineList.recipesList}">
				<td><c:out value="${recipeList.id}" /></td>
				<td><c:out value="${recipeList.name}" /></td>
				<td><c:out value="${recipeList.description}" /></td>
				<td><c:out value="${recipeList.isVeg}" /></td>
				<td><c:out value="${recipeList.price}" /></td>
				<td><c:out value="${recipeList.imageName}" /></td>
				</c:forEach>
